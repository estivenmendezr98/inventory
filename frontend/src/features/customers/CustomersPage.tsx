...
import { CustomerFormModal, type CustomerDto } from './CustomerFormModal';
...
export function CustomersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await apiFetch<ListResponse>(`/customers?${params.toString()}`);
      setRows(res.data);
      setMeta(res.meta);
    } catch (e) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Documento único; CAJERO puede crear; solo SUPER desactiva.
          </p>
        </div>
        {canCreate && (
          <button
                    ...
                    <tr key={c.id} className="border-b border-border/60 hover:bg-muted/20">
                      <td className="p-3">
                        <span className="text-xs text-muted-foreground">
                          {docTypeLabel(c.documentType)}
                        </span>
                        <div className="font-mono text-xs font-medium">{c.documentNumber}</div>
                      </td>
                      <td className="p-3 font-medium">{c.name}</td>
                      <td className="p-3 text-muted-foreground hidden md:table-cell">
                        {c.city ?? '—'}
                      </td>
                      <td className="p-3 text-right tabular-nums">{c.saleCount ?? 0}</td>
                          ...
                          <div className="flex justify-end gap-1">
                            ...
                            {canDelete && c.isActive && (
                              <button
                                ...
                                title="Desactivar"
                                onClick={async () => {
                                  if (!window.confirm(`¿Desactivar a "${c.name}"?`)) return;
                                  try {
